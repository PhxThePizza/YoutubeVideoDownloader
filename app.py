...
def normalize_youtube_url(url_or_id):
  ...
  if len(url_or_id) == 11 and url_or_id.replace('-', '').replace('_',
                                                                 '').isalnum():
...
def cleanup_downloads():
  """Function to delete files older than 30 minutes."""
  ...
  threading.Timer(60, cleanup_downloads).start()


if __name__ == '__main__':
  # Start the cleanup thread
  threading.Thread(target=cleanup_downloads).start()
